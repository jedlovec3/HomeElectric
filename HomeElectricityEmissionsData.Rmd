...
hourly_emissions_365 <- hourly_emissions %>% 
  filter(datetime >= as.POSIXct('04/16/2022 00:00',format="%m/%d/%Y %H:%M",tz=Sys.timezone()) & datetime < as.POSIXct('04/16/2023 00:00',format="%m/%d/%Y %H:%M",tz=Sys.timezone()))
...
```{r}
hourly_emissions_365_date <- hourly_emissions_365 %>% 
  mutate(date = date(datetime)) %>% 
  group_by(date) %>% 
  summarize(net_kWh = sum(net_kWh), produced_kWh = sum(produced_kWh), consumed_kWh = sum(consumed_kWh), net_emissions_co2_kg = sum(net_emissions_co2_kg), produced_emissions_co2_kg = sum(produced_emissions_co2_kg), consumed_emissions_co2_kg = sum(consumed_emissions_co2_kg)) %>% 
  mutate(pct_reduction = 1 - (consumed_emissions_co2_kg - produced_emissions_co2_kg)/consumed_emissions_co2_kg)

hourly_emissions_365_date

ggplot(hourly_emissions_365_date, aes(date,net_emissions_co2_kg)) +
  geom_point() 
...
----------
Find windows of high emissions
```{r}
hourly_emissions %>% 
  filter(year(datetime) == 2023 & month(datetime) == 2 & day(datetime) == 3) %>% 
  arrange(datetime)

hourly_emissions %>% 
  arrange(desc(consumed_kWh))

#sample <- 
hourly_emissions %>% 
  filter(date(datetime) == "2023-08-09")

ggplot(hourly_emissions, aes(as_hms(datetime), MOER*0.45/1000*consumed_kWh)) +
  geom_point()
