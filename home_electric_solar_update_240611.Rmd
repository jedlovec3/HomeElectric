...
ppl_net_export <- hourly_ppl_net %>% 
  select(date, `Read Type`, time, kWh) %>% 
  arrange(date, time) 

#write_csv(ppl_net_export, "ppl_net.csv")
...
consumed_export <- hourly_electricity %>% 
  mutate(kWh = consumed_kWh, `Read Type` = "kWh Consumed") %>% 
  select(date, `Read Type`, time, kWh) %>% 
  arrange(date, time) 

write_csv(consumed_export, "consumed.csv")
